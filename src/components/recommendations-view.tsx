...
export function RecommendationsView({
  ...
  const refresh = async () => {
    setLoading(true);
    try {
      setRecommendations(await getRecommendations(true));
    } catch (err) {
    ...
    <div className="space-y-6">
      ...
      {!hasActivities ? (
        <div className="rounded-2xl border border-border bg-card p-8 text-sm text-muted-foreground">
          Log your first activity to unlock personalized recommendations —
          the AI needs something concrete to base suggestions on.
        </div>
      ) : recommendations.length === 0 ? (
        <div className="rounded-2xl border border-border bg-card p-8 text-sm text-muted-foreground">
          Couldn't load recommendations right now. Try refreshing in a
          moment.
        </div>
      ) : (
